...
/** classe modifiée par Christine Jacquin le 28/09/10
 * principalement changement des signatures de certaines méthodes au niveau de la portée
...
public class C99LINA {
	private final static class Region {
		 ...
		 * @param R Region
		 */
		private final static void bestBoundary_max(final float[][] M, Region R) {
...
/**
 * Mis en commentaire par Christine Jacquin le 28/09/10 précisé par Nicolas Hernandez 11/03/04 
...
//	// KK String[][] D = Stringx.tokenize(LineInput.load(new LineInput(System.in)), " ");
//	String[][] D = null;
//	try { D = Stringx.tokenize(LineInput.load(new LineInput(new java.io.File("/home/jacquin-c/application/un-lina-uima-user-dev-env-20100709-stndln/withoutUIMATS/example.txt"))), " "); }
...
//modification par Christine Jacquin le 28/09/10 
//avant: méthode private final static, maintenant=> protected
protected   ContextVector[] normalize(final String[][] S, ContextVector tf) {
	//System.out.println("on passe pas dans la bonne normalise");
