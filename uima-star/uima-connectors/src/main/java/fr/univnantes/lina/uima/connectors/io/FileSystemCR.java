 ...
 * <li><code>InputCSVDirectory</code> - path to directory containing csv files</li>
 ...
 * 
 */
public class FileSystemCR extends JCasCollectionReader_ImplBase {
	/**
	 ...
	 * csv files.
	 */
	public static final String PARAM_INPUTCSVDIR = "InputCSVDirectory";
	...
	private ArrayList<File> mcsvFiles;
	private HashMap<String,File> mcsvMapFiles;
	...
	public void initialize(final UimaContext context) throws ResourceInitializationException {
		...
		File csvdirectory = new File(((String) getConfigParameterValue(PARAM_INPUTCSVDIR)).trim());
		...
		if (!csvdirectory.exists() || !csvdirectory.isDirectory()) {
			throw new ResourceInitializationException(ResourceConfigurationException.DIRECTORY_NOT_FOUND,
					new Object[] { PARAM_INPUTDIR, this.getMetaData().getName(), csvdirectory.getPath() });
		}

		// get list of files in the specified directory, and subdirectories if the
		// parameter PARAM_SUBDIR is set to True
		mFiles = new ArrayList<File>();
		mFiles  = addFilesFromDir(directory);

		// get list of files in the specified directory, and subdirectories if the
		// parameter PARAM_SUBDIR is set to True
		mcsvFiles = new ArrayList<File>();
		mcsvFiles = addFilesFromDir(csvdirectory);
		for (File f : mcsvFiles) {
			mcsvMapFiles.put(f.getName(),f);
			System.out.println("Debug:f.getName()"+f.getName());  	
		}
	}
	 ...
	 * @param dir
	 */
	private ArrayList<File> addFilesFromDir(File dir) {
	...
	public void getNext(JCas jcas) throws IOException, CollectionException {
		...
		// nh
		// File csvfile = mcsvMapFiles.get(file.getName());
		//CSVReader reader = new CSVReader(new FileReader(csvfile)); //"yourfile.csv"
		//List<String[]> myEntries = reader.readAll();
		//Iterator myEntriesIterator = myEntries.iterator();

		//while (myEntriesIterator.hasNext()) {
		//	String [] nextLine = (String[]) myEntriesIterator.next();
