...
public class JTextTileLINA {
	...
	protected Hashtable stemOf = new Hashtable(); // Word -> stem
...
public static void main(String[] args) {
	...
	protected void similarityDetermination() {
		...
		int i; // Counter
		//System.err.println("Debug: w "+w);
		//System.err.println("Debug: end "+end);

		for (i=w; i<end; i++) {

			/* Compute score for a step */
			if (step == 0) {
				//System.err.println("Debug: score.addElement(new Float(blockCosine(left, right)));");
		...
		//	System.err.println("Debug: step "+step);

		/* Smoothing with a window size of 3 */
		// nh add the following condition 
		//System.err.println("Debug: score.size() "+score.size());
		if (score.size()>2) {
			...
			sim_score = new float[score.size()-1];
			site_loc = new int[site.size()-1];
			for (int j=0; j<sim_score.length; j++) {
