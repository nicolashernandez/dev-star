...
# -----------------------------------------------------------------------------
TEST=QEvsFlagI
INPUTFILE="${ROOT_HOME}/resources/simpleTokenizer/benchmark/QEvsFlagI.txt"
...
do 
  cat $INPUTFILE >> $INPUTTESTFILE
done 
echo "test $TEST - test file done $INPUTTESTFILE"  >> $LOGFILE



#$JAVACMD $JAVAARGS -jar $FTBPLUSPLUS_HOME/lib/ftb++-*.jar $@
