 ...
 *
 */
public class BerkeleyLanguageModelMain {

	static void usage () {
		System.out.println("Usage:");
		System.out.println("\t"+ new BerkeleyLanguageModelMain().getClass().getName() + " estimateLm <order> <inputTrainFile> <outputLMFile>");
		System.out.println("\t"+ new BerkeleyLanguageModelMain().getClass().getName() + " scoreSentences <inputLMFile> <inputSentencesFile> <outputScoreFiles>");
		System.out.println("\t"+ new BerkeleyLanguageModelMain().getClass().getName() + " scoreNgrams <inputLMFile> <inputNgramsFile> <outputScoreFiles>");
		System.exit(1);
	}
	...
	public static void main(String[] args) {
		...
		LanguageModel blm = new BerkeleyLanguageModel();		
		...
		while (true) {
			if (args[i].equalsIgnoreCase("estimateLm")) {
				...
				estimate(i,args,blm); i=i+3;
			}
			else 
				if (args[i].equalsIgnoreCase("scoreSentences")) {
					if (i+3 >= args.length)	usage();
					scoreSentences(i,args,blm);i=i+3;
				}
				else if (args[i].equalsIgnoreCase("scoreNgrams")) {
					if (i+3 >= args.length)	usage();
					scoreNgrams(i,args,blm);i=i+3;
				} else {
					usage();
